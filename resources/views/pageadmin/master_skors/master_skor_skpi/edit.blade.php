                ...
                <div class="card border-top border-0 border-4 border-primary">
                    <div class="card-body p-5">
                        <div class="card-title d-flex align-items-center">
                            ...
                            <h5 class="mb-0 text-primary">Edit Master TINGKAT</h5>
                        </div>
                        <hr>
                        <form action="{{ route('skor.update', $skor->id) }}" method="POST" class="row g-3">
                            @csrf
                            @method('PUT')
                            <div class="col-md-12">
                                ...
                                <select class="form-control" id="unsur_id" name="unsur_id" required>
                                    ...
                                    @foreach ($unsur as $item)
                                        <option value="{{ $item->id }}" {{ $skor->unsur_id == $item->id ? 'selected' : '' }}>
                                            {{ $item->nama_unsur }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="text" class="form-control" id="nama_sub_unsur" name="nama_sub_unsur" value="{{ old('nama_sub_unsur', $skor->nama_sub_unsur) }}">
                            </div>
                            <div class="col-md-12">
                                <label for="nama_tingkat" class="form-label">Nama Tingkat SKPI</label>
                                <label for="" class="form-label text-danger">*kosongkan jika tidak ada</label>
                                <input type="text" class="form-control" id="nama_tingkat" name="nama_tingkat" value="{{ old('nama_tingkat', $skor->nama_tingkat) }}">
                            </div>
                            <div class="col-md-12">
                                <label for="skor" class="form-label">SKOR</label>
                                <input type="number" class="form-control" id="skor" name="skor" value="{{ old('skor', $skor->skor) }}" required>
                            </div>
