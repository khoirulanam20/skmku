                ...
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Pendaftaran Skripsi Jurnal</li>
                        </ol>
                    </nav>
                </div>
            </div>
            <!--breadcrumb-->
            <h6 class="mb-0 text-uppercase">Data Pendaftaran Skripsi Jurnal</h6>
            <hr />
            <div class="card">
                <div class="card-body">
                    <a href="{{ route('pendaftaranskripsijurnal.create') }}" class="btn btn-primary mb-3">Tambah Data</a>
                    <div class="table-responsive">
                        <table id="example2" class="table table-striped table-bordered">
                            ...
                            <tbody>
                                @foreach ($pendaftaranSkripsisJurnal as $index => $p)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        ...
                                        <td>
                                            @if ($p->status === 'pending')
                                                <a href="{{ route('pendaftaranskripsijurnal.edit', $p->id) }}"
                                                    class="btn btn-sm btn-warning">Edit</a>
                                            @endif
                                            <form action="{{ route('pendaftaranskripsijurnal.destroy', $p->id) }}" method="POST"
                                            ...
                                            @if ($p->status === 'diterima')
                                                ...
                                                <a href="{{ route('downloadskripsijurnal.c1', $p->id) }}" target="_blank">C1. LAYOUT SKRIPSI</a><br>
                                                <a href="{{ route('downloadskripsijurnal.c2', $p->id) }}" target="_blank">C2. FILE PERBAIKAN PENGUJI DAN PEMBIMBING</a><br>
                                        ...
                                        <td>
                                            ...
                                            @endif
                                            @if ($p->ss_reputasi_jurnal_sinta)
                                                <a href="{{ asset('dokumen/ss_reputasi_jurnal_sinta/' . basename($p->ss_reputasi_jurnal_sinta)) }}"
                                                    target="_blank">Bukti Screenshot Reputasi Jurnal (Sinta)</a><br>
                                            @endif
                                            @if ($p->bukti_publikasi)
                                                <a href="{{ asset('dokumen/bukti_publikasi/' . basename($p->bukti_publikasi)) }}"
                                                    target="_blank">Bukti Publikasi</a><br>
                                            @endif
                                            @if ($p->bukti_koreapondensi)
                                                <a href="{{ asset('dokumen/bukti_koreapondensi/' . basename($p->bukti_koreapondensi)) }}"
                                                    target="_blank">Bukti Koreapondensi</a><br>
                                            @endif
                                        </td>
